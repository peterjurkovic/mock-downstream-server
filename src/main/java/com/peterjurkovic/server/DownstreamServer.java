...
public class DownstreamServer {
	...
	public void start() {
		wireMockServer.stubFor(get(urlEqualTo("/ping")).willReturn(aResponse().withBody("DATA").withFixedDelay(100)));

		wireMockServer.stubFor(
				get(urlEqualTo("/slow-endpoint"))
					.willReturn(aResponse()
					.withHeader("Content-Type", "application/json")
					// this is needed due to bug
					// https://github.com/ratpack/ratpack/issues/1058
					.withHeader("Content-length", String.valueOf(USER_JSON.length()))
					.withBody(USER_JSON)
					.withFixedDelay(100)));

		wireMockServer.stubFor(
				get(urlEqualTo("/fast-endpoint"))
					.willReturn(aResponse()
					.withHeader("Content-Type", "application/json")
					// this is needed due to bug
					// https://github.com/ratpack/ratpack/issues/1058
